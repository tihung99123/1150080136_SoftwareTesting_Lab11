...
- `bai5/` is the standalone folder for Allure Report nâng cao với @Feature/@Story/@Severity, Allure.step(), ScreenshotOnFailureListener.
- `bai6/` is the pipeline upgrade — Full CI with GitHub Pages publish via `selenium-full.yml`.
...
- **Job `test`**: matrix [chrome, firefox], chạy `bai5` smoke tests, upload `allure-results-{browser}` artifacts
- **Job `publish-report`**: download cả 2 artifacts → generate Allure Report → deploy to `gh-pages` branch
